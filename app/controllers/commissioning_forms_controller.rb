...
@commissioning_form = CommissioningForm.includes([:mods,:inverters]).find(params[:id])
 ...
 #end
 @commissioning_form.user = current_user
  ...
  private
    ...
    def commissioning_form_params
      params.require(:commissioning_form).permit(:user_id,:draft, :send_email, :commissioning_date, :employee_name, :job, :customer_name, :job_type, :system_size, :location, :physical_address, :mailing_address, :parcel_id, :customer_phone, :customer_email, :permit, :installation_completed, :net_meter_installed, :wapa_account, :project_notes,
